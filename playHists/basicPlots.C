...
void basicPlots::drawHists( bool MuAddOrNot, TString HTBins, int whichpart, int rebin, TString xAxisName, TString yAxisName, double xAxisRange1, double xAxisRange2, TString whichplot, TLegend * len, double lowy, double highy, int OneDTwoD, int startNJet, int nJets ){
  ...
  if( hasMCtotal_ ){
    ...
    if( hasZinvFromDY_ && whichpart == 1){
      TH1D *MCh_ZinvFromDY= (getHists( MuAddOrNot, HTBins, whichpart, rebin, xAxisName, yAxisName, xAxisRange1, xAxisRange2, 2, whichplot, true, "ZinvFromDY", lowy, highy, OneDTwoD, startNJet, nJets ))[0];
  ...
  if( hasWJ_ ){
    ...
    if( hasWJveryHighHT_ && ( HTBins == "highHTBins" || HTBins == "all") ){
     TH1D *MCh_WJveryHighHT= (getHists( MuAddOrNot, "highHTBins", whichpart, rebin, xAxisName, yAxisName, xAxisRange1, xAxisRange2, 2, whichplot, true, "WJveryHighHT", lowy, highy, OneDTwoD, startNJet, nJets ))[0];
     MCh_WJ->Add(MCh_WJ,MCh_WJveryHighHT);
    }
    vh.push_back(MCh_WJ);
