...
//================
  ...
  HTTakeMuCut::HTTakeMuCut(float sumEt)
    : mSumEtCutVal(sumEt)
  {;}

  bool HTTakeMuCut::Process(Event::Data & ev) {
    return (ev.CommonHTTakeMu() > mSumEtCutVal);
  }

  std::ostream& HTTakeMuCut::Description(std::ostream &ostrm) {
    ostrm << "HT " << mSumEtCutVal << " GeV ";
    return ostrm;
  }
  ...
  //================
  // HT(jet and mu) LessThan cut
  //================

  HTTakeMuLessThanCut::HTTakeMuLessThanCut(float sumEt)
    : mSumEtCutVal(sumEt)
  {;}

  bool HTTakeMuLessThanCut::Process(Event::Data & ev) {
    return (ev.CommonHTTakeMu() < mSumEtCutVal);
  }

  std::ostream& HTTakeMuLessThanCut::Description(std::ostream &ostrm) {
    ostrm << "HT less than" << mSumEtCutVal << " GeV ";
    return ostrm;
  }
...
//================
  ...
  //===========
  // Alpha_t Cut Take muon into account
  //===========
  alpha_tTakeMuCut::alpha_tTakeMuCut(float value) :
  mCut(value) {}

  alpha_tTakeMuCut::~alpha_tTakeMuCut() {}

  bool alpha_tTakeMuCut::Process(Event::Data & ev) {
    if(ev.JD_CommonJets().accepted.size() + ev.LD_CommonMuons().accepted.size() < 2) return false;
    return ( ev.HadronicAlphaTTakeMu()> mCut);
  }

  std::ostream& alpha_tTakeMuCut::Description(std::ostream &ostrm) {
    ostrm << "Alpha_t TakeMu  > " << mCut << " GeV): ";
    return ostrm;
  }


  //===========
  // Alpha_t Cut Less than value Take muon into account
  //===========
  alpha_tTakeMuCut_Less::alpha_tTakeMuCut_Less(float value) :
  mCut(value) {}

  alpha_tTakeMuCut_Less::~alpha_tTakeMuCut_Less() {}

  bool alpha_tTakeMuCut_Less::Process(Event::Data & ev) {
    if(ev.JD_CommonJets().accepted.size() + ev.LD_CommonMuons().accepted.size() < 2) return false;
    return ( ev.HadronicAlphaTTakeMu()< mCut);
  }

  std::ostream& alpha_tTakeMuCut_Less::Description(std::ostream &ostrm) {
    ostrm << "Alpha_t TakeMu  < " << mCut << " GeV): ";
    return ostrm;
  }
